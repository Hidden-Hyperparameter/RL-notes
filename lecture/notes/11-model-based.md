...
- 模型**本身**的不确定性（model uncertainty）：这才是我们之前提到的，因为**数据采集不够或者分布不对**造成的模型的bias。比如说DL里面讲的“cook问题”：如果训练数据集里面大部分做饭的人都是女性，那么模型在预测做饭的人的性别的时候，很可能非常confident地认为这人是女性。注意，这种不确定性是**不能**从模型的输出的logits中看出来的。
...
而现在不同时间的$\theta$完全是一样的。但实际上，这个方法work！我目前还完全无法理解这一点。希望看到这里的高手可以为我指点迷津。
